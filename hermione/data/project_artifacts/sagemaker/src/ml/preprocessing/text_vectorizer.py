...
class TextVectorizer:
    ...
    def fit(self, df: pd.DataFrame):
        ...
        for vectorizer in self.vectorizer_cols:
            ...
            for col in self.vectorizer_cols[vectorizer]:
                ...
                self.vectorizers_fitted[vectorizer][col] = self.vectorizer_vects[
                    vectorizer
                ](df[col].values)
        self.fitted = True
    ...
    def embedding(self, X, typ_transform=1):
        ...
        if typ_transform == 1:  # mediana
            vector = np.median(
                [self.word2vec[x] for x in X.split() if x in self.word2vec], axis=0
            )
    ...
    def inverse_transform(self, df: pd.DataFrame):
        ...
        for vectorizer in self.vectorizer_cols:
            if vectorizer == "index":
                for col in self.vectorizer_cols[vectorizer]:
                    df.loc[:, col + "_remove_" + vectorizer] = df[col].apply(
                        lambda x: self.unvectorize(x)
                    )
            elif (vectorizer == "bag_of_words") | (vectorizer == "tf_idf"):
                for col in self.vectorizer_cols[vectorizer]:
                    values = self.vectorizers_fitted[vectorizer][col].inverse_transform(
                        df[col]
                    )
                    df.loc[:, col + "_remove_" + vectorizer] = pd.Series(
                        values.toarray().tolist()
                    )
