...
class Normalizer:
    def __init__(self, norm_cols: dict):
        ...
        self.norms = {"min-max": MinMaxScaler, "standard": StandardScaler}
        self.fitted = False
    ...
    def statistics(self, df: pd.DataFrame):
        ...
        self.col_mean = {
            col: value for col, value in zip_cols(df[self.col_names].mean())
        }
        self.col_median = {
            col: value for col, value in zip_cols(df[self.col_names].median())
        }
    ...
    def fit(self, df: pd.DataFrame):
        ...
        for norm in self.norm_cols:
            ...
            for col in self.norm_cols[norm]:
                self.normalization[col] = self.__apply_func(
                    df[col].values.reshape(-1, 1), self.norms[norm]()
                )
        self.fitted = True
    ...
    def transform(self, df: pd.DataFrame):
        ...
        for norm in self.norm_cols:
            if norm == "zscore":
                for col in self.norm_cols[norm]:
                    df.loc[:, col] = (
                        df[col].values - self.col_mean[col]
                    ) / self.col_std[col]
            elif norm == "log10":
                for col in self.norm_cols[norm]:
                ...
                for col in self.norm_cols[norm]:
                    df.loc[:, col] = self.normalization[col].transform(
            ...
            if norm == "zscore":
                for col in self.norm_cols[norm]:
                    df.loc[:, col] = df[col].apply(
                        lambda z: self.col_std[col] * z + self.col_mean[col]
                    )
            elif norm == "log10":
                for col in self.norm_cols[norm]:
                ...
                for col in self.norm_cols[norm]:
                    df.loc[:, col] = self.normalization[col].inverse_transform(
                        df[col].values.reshape(-1, 1)
                    )
        return df
