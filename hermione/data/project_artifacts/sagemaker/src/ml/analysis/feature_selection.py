...
class SelectCoefficients(SelectAlgorithm):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        self.selected_columns = np.argsort(np.abs(trained_model.coef_.ravel()))[
            -self.num_feat :
        ]
...
class SelectCorrelation(SelectAlgorithm):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        [
            self.check_correlation(corr.iloc[i, j], j)
...
class SelectEnsemble(SelectAlgorithm):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        selections = [
            FeatureSelector(selector, **self.dic_selection[selector])
            for selector in self.dic_selection
        ]
        [selection.fit(X, y) for selection in selections]
        [
            self.increment_count(column)
