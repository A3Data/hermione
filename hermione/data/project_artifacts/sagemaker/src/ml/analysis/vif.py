...
class VIF:
    ...
    def analyze(cls, df: pd.DataFrame, thresh=5.0, verbose=True):
        ...
        while dropped:
            dropped = False
            vif = [
                variance_inflation_factor(df.iloc[:, variables].values, ix)
            ...
            if max(vif) > thresh:
                ...
                if verbose:
                    cols_possibles_remove = [
                        str(df.iloc[:, variables].columns[i]) for i in index_max
                    ]
                    print(
                        "Columns that can be removed -> "
                        + ", ".join(cols_possibles_remove)
                    )
                    print("------")
                print(
                    "dropping '"
                    + str(df.iloc[:, variables].columns[maxloc])
                    + "' at index: "
                    + str(maxloc)
                )
