...
class PCAReducer:
    ...
    def __find_k(self, df, threshold):
        ...
        int
        """
        self.pca = PCA(n_components=len(self.columns))
        ...
        for i in range(len(self.columns) - 1):
            if (
                ...
                if i == 0:
                    raise Exception("Not reduced by poor explicability")
                return i + 1
    ...
    def report(self):
        ...
        for col in range(self.k):
            print(
                "Explained variance ({col}): {ratio}".format(
                    col=self.prefix + "_" + str(col),
