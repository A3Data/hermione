...
class DataDrift:
    """
    Class to perform assess data drift before training
    """
    ...
    def check(self, df: pd.DataFrame, target=None, cut_off=2):
        """
        Assesses data drift
        ...
        if self.continuous_cols != None:
            for col in self.continuous_cols:
                ...
                df_ge.expect_column_mean_to_be_between(
                    col,
                    measures["mean"] - cut_off * measures["std"],
                    measures["mean"] + cut_off * measures["std"],
                )
                df_ge.expect_column_max_to_be_between(
                    col,
                    measures["max"] - cut_off * measures["std"],
                    measures["max"] + cut_off * measures["std"],
                )
                df_ge.expect_column_min_to_be_between(
                    col,
                    measures["min"] - cut_off * measures["std"],
                    measures["min"] + cut_off * measures["std"],
                )
                expected_partition = ge.dataset.util.continuous_partition_data(
                    df_ge[col]
                )
                df_ge.expect_column_bootstrapped_ks_test_p_value_to_be_greater_than(
                    col, expected_partition
                )
        if len(self.discrete_cat_cols) != None:
            for col in self.discrete_cat_cols:
                ...
                expected_partition = ge.dataset.util.categorical_partition_data(
                    df_ge[col]
                )
                df_ge.expect_column_chisquare_test_p_value_to_be_greater_than(
                    col, expected_partition
                )
        return df_ge
