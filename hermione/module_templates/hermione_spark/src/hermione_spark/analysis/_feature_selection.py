...
class SparkFS(Asserter):
    ...
    def transform(self, df):
        ...
        for col_type, params in self.mapping.items():
            assembler = VectorAssembler(inputCols=params['cols'], outputCol=f'{col_type}_features')
            selector = (
            ...
            if 'selectionThreshold' in params:
                self.assert_type(params['selectionThreshold'], (int, float), 'selectionThreshold')
        ...
        assembler = VectorAssembler(inputCols=selected_cols, outputCol='selected_features')
