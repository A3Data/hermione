...
def read_input(file_path):
    ...
    """
    input_files = [os.path.join(file_path, file)
    ...
    if len(input_files) == 0:
        raise ValueError(('There are no files in {}.\n' +
                          ...
                          the data.').format(file_path, channel_name))
...
def train():
    ...
    try:
        ...
        pd.DataFrame(model.artifacts["metrics"].items(),
                     columns=['Metric', 'Value']).to_csv(
            os.path.join(model_path, 'metrics.csv'), index=False)
